...
開発者の Mac 1 台で、同じ発話を 4 つの組み合わせに通して計測した結果です（Apple 音声認識はそのうち 3 回、ほかは 1 回ずつ）。**1 台での少ない試行なので、あなたの環境で同じ数字が出るとは限りません。** 性能の主張ではなく、既定を決めた根拠として書いています。

- 音声認識は Apple が **292ms**、WhisperKit（当時の large-v3）が **3,906ms** でした。しかも Apple 側は**句読点を自前で付けてくる**ので、整形の仕事の相当部分が認識の時点で済みます
- その後 WhisperKit 側は **large-v3-turbo**（約 630MB）に入れ替えたので、いまの差はここまで大きくありません（実機の履歴の中央値で 約 470ms 対 134ms）。上の 3,906ms は、既定を決めたときの値としてそのまま載せています
- Apple の音声認識には弱点があります。英数字・URL・メールアドレスは 3 回とも別々の壊れ方をしました。**壊れ方が毎回違うので辞書置換では救えません。** ここが重要な用途では WhisperKit に切り替えてください（同じ発話で WhisperKit は URL もメールアドレスも正しく取れています）
...
- 空きディスク: 既定（macOS 26 以降）なら**追加で要りません**。WhisperKit なら約 630MB
...
設定は `UserDefaults` に保存されます。**既定値を変えても、すでに保存されている選択は上書きされません。**

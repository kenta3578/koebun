...
`koemakaseTests`（Swift Testing）。`xcodebuild test` はアプリを `TEST_HOST` として起動するが、
...
| `DataDirectoryTests` | 旧名の置き場 `~/koebun` からの移行。両方あるときは触らない |
...
- `~/koemakase/` のファイルを読み書きする `*.shared` シングルトンは使わず、`nonisolated static` な純関数を呼ぶ
